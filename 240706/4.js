...
console.log(arr.filter(el => el.length >= 4))
 
//5.
var arr = ["banana", "kiwi", "mango", "strawberry", "lime", "orange", "plum", "cherry"];
// 요소의 문자열 길이가 홀수인 요소만 반환하여 출력해주세요
console.log(arr.filter(el => el.length % 2 === 1))
...
console.log(student.map(std => std.name === null ? true : false))
console.log(student.some(std => std.name === null))

//9.
var arr = ["banana", "kiwi", "mango", "strawberry", "lime", "orange", "plum", "cherry"];
// 문자열 길이가 5를 초과하는 요소가 있는지 검사하는 코드를 작성하고 반환값을 출력해주세요.
console.log(arr.map(el => el.length > 5 ? true : false))
